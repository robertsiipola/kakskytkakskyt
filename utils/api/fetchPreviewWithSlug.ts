...
const fetchPreviewWithSlug = async (slug: (string | string[]) | null): Promise<Post> => {
    if (slug && typeof slug === 'string') {
        const url = `https://preview.contentful.com/spaces/${process.env.NEXT_PUBLIC_CONTENTFUL_SPACE_ID}/environments/${process.env.NEXT_PUBLIC_CONTENTFUL_ENVIRONMENT_ID}/entries?content_type=blogPost&fields.slug=${slug}`;
        const token = process.env.NEXT_PUBLIC_CONTENTFUL_PREVIEW_ACCESS_TOKEN;
